...
async function loadPokemonToDashboard(species, level, nature, xp, customName = "") {
    try {
        ...
        currentPokemon = await Pokemon.create(species, level, nature, xp);
...
document.getElementById('btn-save-poke').addEventListener('click', () => {
    ...
    saves[customName] = {
        species: currentPokemon.name,
        level: currentPokemon.level,
        xp: currentPokemon.current_xp,
...
function updateUI() {
    ...
    document.getElementById('poke-sprite').src = getSpriteUrl(currentPokemon.id);
    ...
    const xpPercent = currentPokemon.level === 100 
        ? 100 
        : (currentPokemon.current_xp / currentPokemon.xp_to_level) * 100;
